...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        imageView.frame = CGRectMake(i * frame.size.width / self.numberOfStar+5, 5, frame.size.width / self.numberOfStar -10, frame.size.height-10);
        [view addSubview:imageView];
    }
    return view;
}
-(void)setStarValue:(int)value{
    CGFloat each = self.frame.size.width / _numberOfStar;
    [self changeStarForegroundViewWithPoint:CGPointMake(each * value, 0)];
}
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    score = (((int)(score*10))/2 * 2 + 2) / 10.0f;
