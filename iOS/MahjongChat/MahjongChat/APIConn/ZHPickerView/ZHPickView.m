...
NSDateFormatter *pickerFormatter, *datetimeFormatter;

-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isHaveNavControler:(BOOL)isHaveNavControler{
    
    self=[super init];
    if (self) {
        pickerFormatter = [[NSDateFormatter alloc] init];
        [pickerFormatter setTimeZone:[NSTimeZone timeZoneWithName:@"Asia/Taipei"]];
        [pickerFormatter setDateFormat:@"yyyy-MM-dd"];
        datetimeFormatter = [[NSDateFormatter alloc] init];
        [datetimeFormatter setTimeZone:[NSTimeZone timeZoneWithName:@"Asia/Taipei"]];
...
#warning test
    pickView.frame=CGRectMake(0, ZHToobarHeight, WIDTH, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.backgroundColor=[UIColor colorWithWhite:0.98 alpha:0.98];
    if (_defaulDate) {
...
-(void)show{
    if (bgView==nil) {
        bgView = [[UIView alloc] initWithFrame:[UIApplication sharedApplication].keyWindow.frame];
        [bgView setBackgroundColor:[UIColor colorWithWhite:0.2 alpha:0.3f]];
    }
...
-(void)doneClick
{
    ...
    if (_pickerView) {
        ...
        if (pickerMode == UIDatePickerModeDateAndTime) {
            _resultString=[NSString stringWithFormat:@"%@", [datetimeFormatter stringFromDate:_datePicker.date]];
        }else{
            _resultString=[NSString stringWithFormat:@"%@", [pickerFormatter stringFromDate:_datePicker.date]];
        }
    }
